package com.zmsoft.ccd.lib.widget.pickerview.lib;
...
import com.zmsoft.ccd.lib.widget.pickerview.adapter.WheelAdapter;
import com.zmsoft.ccd.lib.widget.pickerview.listener.OnItemSelectedListener;
...
public class WheelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                contentText = getResultContent(contentText);
                //计算开始绘制的位置
     ...
     * 根据文字长度，重新设置文字的内容，显示...
     */
    private String getResultContent(String contentText) {
        Rect rect = new Rect();
        int originContentLength = contentText.length();
        paintCenterText.getTextBounds(contentText, 0, originContentLength, rect);
        int width = rect.width();
        if (width > measuredWidth) { // 如果文字比屏幕宽
            // 获取一个宽度
            int oneWidth = getOneWidth();
            // 可以显示多少个字
            int endIndex = measuredWidth / oneWidth - 2;
            contentText = contentText.substring(0, endIndex > originContentLength ? originContentLength - 2 : endIndex);
            contentText += "...";
        }
        return contentText;
    }

    private int getOneWidth() {
        String text = "默";
        Rect rect = new Rect();
