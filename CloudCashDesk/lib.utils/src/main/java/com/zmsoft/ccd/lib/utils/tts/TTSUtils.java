...
public class TTSUtils {
    ...
    public static void initTts(Context context) {
        // 设置初始化参数
        SpeechSynthesizerListener listener = new UiMessageListener(); // 此处可以改为 含有您业务逻辑的SpeechSynthesizerListener的实现类
        Map<String, String> params = getParams();
        // appId appKey secretKey 网站上您申请的应用获取。注意使用离线合成功能的话，需要应用中填写您app的包名。包名在build.gradle中获取。
        String appId = "10301717";
        String appKey = "GiZVqPlu6mMmy7rWKKskOWA2";
        String secretKey = "96e835ee1bde29837620dbc65209f430";
        InitConfig initConfig = new InitConfig(appId, appKey, secretKey, ttsMode, offlineVoice, params, listener);
        synthesizer = new NonBlockSyntherizer(context.getApplicationContext(), initConfig); // 此处可以改为MySyntherizer 了解调用过程
    }
     ...
     * 批量播放
     */
    public static void batchSpeak(List<Pair<String, String>> textList) {
...
//        texts.add(new Pair<String, String>("重(chong2)量这个是多音字示例", "a3"));
        int result = synthesizer.batchSpeak(textList);
    ...
    private static void checkResult(int result, String method) {
        if (result != 0) {
            Logger.e("TTS error code :" + result + " method:" + method + ", 错误码文档:http://yuyin.baidu.com/docs/tts/122 ");
        }
    }
}
