    ...
    protected boolean init(InitConfig config) {
        ...
        if (isMix) {
            try {
                ...
                e.printStackTrace();
                Logger.e("TTS 【error】:copy files from assets failed." + e.getMessage());
                return false;
            }
        }
        ...
        if (isMix) {
            ...
            Logger.d("TTS 离线资源路径：" + offlineResource.getTextFilename() + ":" + offlineResource.getModelFilename());
            ...
            if (!authInfo.isSuccess()) {
                ...
                Logger.d("TTS 鉴权失败 =" + errorMsg);
                return false;
            } else {
                Logger.d("TTS 验证通过，离线正式授权文件存在。");
            }
        }
        ...
        if (result != 0) {
            Logger.d("TTS 【error】initTts 初始化失败 + errorCode：" + result);
            return false;
        }
        // 此时可以调用 speak和synthesize方法
        Logger.d("TTS 合成引擎初始化成功");
        return true;
    }
