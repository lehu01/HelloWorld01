...
public final class RetailDataMapLayer {
    ...
    /**
     * 将服务的返回的所有桌位数据，转换为选桌recyclerView所需的List
     *
     * @param list 数据源
     * @return 目标数据
     */
    public static List<SelectSeatVo> getSelectAllSeatList(List<SeatArea> list) {
        ...
        for (SeatArea seatArea : list) {
            ...
            for (Seat seat : seatList) {
                ...
                seatVo.setSeat(seat);
                selectSeatVos.add(seatVo);
            }
        }
        return selectSeatVos;
    }

    public static List<SelectSeatVo> getSelectAllSeatList(List<SeatArea> list, String seatCode) {
        List<SelectSeatVo> selectSeatVos = new ArrayList<>();
        if (list == null) {
            return selectSeatVos;
        }
    ...
    public static List<ViewHolderSeat> getAllSeatsList(List<SeatArea> allSeatsResponse, boolean isWatchSeat) {
        ...
        for (int i = 0; i < allSeatsResponse.size(); i++) {
            ...
            if (null != seatList && !seatList.isEmpty()) {
                ...
                ViewHolderSeat areaViewHolderSeat = new ViewHolderSeat(true, seatArea.getAreaId()
                        , seatArea.getAreaName());
    ...
    public static List<DeskMessage> initDeskMsg(List<DeskMessage> deskList) {
        ...
        for (int i = 0; i < deskList.size(); i++) {
            DeskMessage deskMessage = deskList.get(i);
            if (null == deskMessage) {
                continue;
            }
                 /*
                  1、加菜消息，服务端返回的content里包含的是json，有个key为waitingInstanceCount，需要解析这个json取出waitingInstanceCount
                  ,再组装为content显示，其他类型消息直接使用content显示。
                  2、其他类型消息
                  返回的content内容会有个类似“1|”的前缀，需要把这个前缀去掉
                 */
            //如果是加菜消息，需要解析cot字段里的json获取想要的文本内容
            if (deskMessage.getTy() == MessageHelper.MsgType.TYPE_ADD_FOOD_SCAN_ORDER
                ...
                int foodCount = 0;
                DeskMsgAddFoodNum deskMsgAddFoodNum = JsonMapper.fromJson(deskMessage.getCot()
                        , DeskMsgAddFoodNum.class);
                if (null != deskMsgAddFoodNum) {
                    foodCount = deskMsgAddFoodNum.getWaitingInstanceCount();
                    deskMessage.setOrderCode(deskMsgAddFoodNum.getOrderCode());
                }
                if (deskMessage.getTy() == MessageHelper.MsgType.TYPE_ADD_FOOD_SCAN_ORDER) {
                    deskMessage.setCot(String.format(context.getResources().getString(R.string.msg_msgcenter_addfood_scan_order_content)
                            , foodCount));
                } else if (deskMessage.getTy() == MessageHelper.MsgType.TYPE_ADD_FOOD_SCAN_DESK) {
                    deskMessage.setCot(String.format(context.getResources().getString(R.string.msg_msgcenter_addfood_scan_desk_content)
                            , foodCount));
                } else if (deskMessage.getTy() == MessageHelper.MsgType.TYPE_PREPAY_SCAN_DESK) {
                    String feeStr = "0";
                    String payStatus = null;
                    if (null != deskMsgAddFoodNum) {
                        double fee = deskMsgAddFoodNum.getFee();
                        if (fee >= 0) {
                            feeStr = FeeHelper.getDecimalFee(fee);
                        }
                        payStatus = deskMsgAddFoodNum.getPayStatus();
                    }
                    deskMessage.setCot(String.format(context.getResources().getString(R.string.msg_msgcenter_prepay_scan_desk_content)
                            , foodCount, feeStr, payStatus));
                } else if (deskMessage.getTy() == MessageHelper.MsgType.TYPE_AUTO_CHECK_PRE_PAY) {
                    deskMessage.setCot(String.format(context.getResources().getString(R.string.msg_msgcenter_addfood_scan_desk_content)
                            , foodCount));
                } else {
                    deskMessage.setCot(String.format(context.getResources().getString(R.string.msg_msgcenter_autocheck_scan_desk_content)
                            , foodCount));
                }
            } else {
                String content = deskMessage.getCot();
                ...
                if (null != serviceBillVO) {
                    ...
                    if (serviceBillVO.getFinalAmount() > 0) {
                        ...
                        receivableFee = String.format(context.getResources()
                                .getString(R.string.msg_detail_price_format), receivableFee);
                    }
                    ...
                    if (serviceBillVO.getPaidFee() > 0) {
                        ...
                        receivedFee = String.format(context.getResources()
                                .getString(R.string.msg_detail_price_format), receivedFee);
                    }
                    ...
                    if (serviceBillVO.getNeedPayFee() != 0) {
                        ...
                        exceedFee = String.format(context.getResources()
                                .getString(R.string.msg_detail_price_format), exceedFee);
                        if (serviceBillVO.getNeedPayFee() > 0) {
                    ...
                    if (serviceBillVO.getOriginAmount() > 0) {
                        ...
                        consumeFee = String.format(context.getResources()
                                .getString(R.string.msg_detail_price_format), consumeFee);
                    }
                    ...
                    if (serviceBillVO.getOriginServiceCharge() > 0) {
                        ...
                        serviceFee = String.format(context.getResources()
                                .getString(R.string.msg_detail_price_format), serviceFee);
                    }
                    ...
                    if (serviceBillVO.getOriginLeastAmount() > 0) {
                        ...
                        minimumFee = String.format(context.getResources()
                                .getString(R.string.msg_detail_price_format), minimumFee);
                    }
                    ...
                    if (serviceBillVO.getDiscountAmount() != 0) {
                        ...
                        if (serviceBillVO.getDiscountAmount() < 0) {
                            discountFee = String.format(context.getResources()
                                    .getString(R.string.msg_detail_price_negative_format), discountFee);
                        } else {
                            discountFee = String.format(context.getResources()
                                    .getString(R.string.msg_detail_price_format), discountFee);
                        }
                    }
                    ...
                    if (null != deskMsgDetailFood) {
                        switch (deskMsgDetailFood.getKind()) {
                            ...
                            case MessageDetailHelper.FoodType.TYPE_NORMAL_FOOD:
                                ...
                                foodsItemInfo.setPrice(String.format(context.getResources().getString(R.string.msg_detail_price_format), fee));
                            ...
                            case MessageDetailHelper.FoodType.TYPE_COMBO_FOOD:
                                ...
                                deskMsgDetailComboFoodsTitleRecyclerItem.setFoodPrice(String.format(context.getResources().getString(R.string.msg_detail_price_format), comboFee));
        ...
        if (null != takeoutOrderVo) {
            ...
            switch (takeoutOrderVo.getOrderFrom()) {
                ...
                case TakeoutConstants.OrderFrom.WEIDIAN:
                    orderInfoRecyclerItem.setSource(context.getString(R.string.module_takeout_order_from_name_weidian));
                    break;
            }
            switch (takeoutOrderVo.getSendType()) {
                ...
                case TakeoutConstants.SendType.SELF_TAKE:
                    orderInfoRecyclerItem.setDeliveryMethod(context.getString(R.string.retail_takeout_take_self));
                    break;
            }
            ...
            switch (takeoutOrderVo.getReserveStatus()) {
                case TakeoutConstants.ReserveStatus.DELIVERY_IMMEDIATELY:
                    if (takeoutOrderVo.getSendType() == TakeoutConstants.SendType.SELF_TAKE) {
                        orderInfoRecyclerItem.setSendTime(String.format(context.getResources().getString(R.string.retail_takeout_takeself_set_time)
                        ...
                        if (null != detailFood) {
                            switch (detailFood.getKind()) {
                                ...
                                case MessageDetailHelper.FoodType.TYPE_NORMAL_FOOD:
                                    ...
                                    foodsItemInfo.setAccountUnit(detailFood.getAccountUnit());
                                    foodsItemInfo.setCode(detailFood.getCode());
                                    foodsItemInfo.setUnit(detailFood.getUnit());
                                    foodsItemInfo.setDoubleUnits(detailFood.getDoubleUnits());
