...
import com.zmsoft.ccd.module.main.order.detail.RetailOrderDetailPresenter;
 ...
 */
public class RetailOrderDetailFragment extends BaseListFragment implements RetailOrderDetailContract.View {
    ...
    @BindView(R.id.text_print_dishes_order)
    ...
    public static RetailOrderDetailFragment newInstance(int from, String orderId, OrderDetail orderDetail) {
        RetailOrderDetailFragment fragment = new RetailOrderDetailFragment();
        Bundle bundle = new Bundle();
        bundle.putInt(RouterPathConstant.RetailOrderDetail.EXTRA_FROM, from);
        bundle.putSerializable(RouterPathConstant.RetailOrderDetail.EXTRA_ORDER_DETAIL, orderDetail);
        bundle.putString(RouterPathConstant.RetailOrderDetail.EXTRA_ORDER_ID, orderId);
    ...
    protected int getLayoutId() {
        return R.layout.fragment_retail_order_detail;
    }
    ...
    protected BaseListAdapter createAdapter() {
        mOrderDetailAdapter = new RetailOrderDetailAdapter(getActivity(), null);
...
//        });
    ...
    @OnClick({R.id.text_pay_state, R.id.text_add_instance, R.id.text_order_op, R.id.text_cancel_order, R.id.text_print_dishes_order})
        ...
        if (hasInstance()) {
            if (BatchPermissionHelper.getPermission(Permission.ReprintSmallTicket.ACTION_CODE)) {
                CcdPrintHelper.reprintOrder(getActivity()
                        , OrderHelper.isTakeoutOrder(mOrderDetail.getOrder().getOrderFrom()) //零售打印区分外卖单和本地收银
                                ? PrintData.BIZ_TYPE_PRINT_RETAIL_ORDER : PrintData.BIZ_TYPE_PRINT_RETAIL_FINANCE_ORDER
                        , mOrderId);
                ToastUtils.showShortToast(getContext(), R.string.module_takeout_print_waiting);
            } else {
                showToast(getString(R.string.retail_permission_print_account));
            }
        }
    }
    ...
    private void updateOpOrderView(OrderDetail orderDetail) {
        ...
        if (OrderHelper.isEndPay(orderDetail.getOrder().getStatus())) {
            ...
            mRlPrintOrder.setVisibility(View.VISIBLE);
            mRlCancelOrder.setVisibility(View.GONE);
            setTextPayStateLayoutParams(RelativeLayout.ALIGN_PARENT_RIGHT);
        } else {
            // 外卖单不能操作
            if (OrderHelper.isTakeoutOrder(mOrderDetail.getOrder().getOrderFrom())) {
                mTextAddInstance.setVisibility(View.GONE);
                ...
                mRlCancelOrder.setVisibility(View.GONE);
                mRlPrintOrder.setVisibility(View.GONE);
            } else {
                mTextAddInstance.setVisibility(View.GONE);
                mTextOrderOp.setVisibility(View.GONE);
                mRlPrintOrder.setVisibility(View.GONE);
                mRlCancelOrder.setVisibility(View.VISIBLE);
                setTextPayStateLayoutParams(RelativeLayout.ALIGN_PARENT_LEFT);
            }
        }
    }

    private void setTextPayStateLayoutParams(int verb) {
        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mTextPayState.getLayoutParams();
        params.addRule(verb);
        if (verb == RelativeLayout.ALIGN_PARENT_RIGHT) {
            params.setMargins(0, 0, ConvertUtils.dp2px(getActivity(), 30.f), 0);
        } else {
            params.setMargins(ConvertUtils.dp2px(getActivity(), 30.f), 0, 0, 0);
        }
        mTextPayState.setLayoutParams(params);
    }
...
//        mFeePlanPickerView.show();
    ...
    private void gotoCancelOrderActivity() {
        MRouter.getInstance().build(RouterPathConstant.RetailCancelOrder.PATH)
                .putString(RouterPathConstant.RetailCancelOrder.EXTRA_ORDER_ID, mOrderId)
                .putString(RouterPathConstant.RetailCancelOrder.EXTRA_SEAT_NAME, mOrderDetail.getSeatName())
                .putString(RouterPathConstant.RetailCancelOrder.EXTRA_ORDER_SERIAL_NUMBER, mOrderDetail.getOrder().getInnerCode())
                .putInt(RouterPathConstant.RetailCancelOrder.EXTRA_ORDER_NUMBER, mOrderDetail.getOrder().getCode())
                .putLong(RouterPathConstant.RetailCancelOrder.EXTRA_MODIFY_TIME, mModifyTime)
                .navigation(getActivity(), RetailOrderDetailActivity.RESULT_CANCEL_ORDER);
    }
                    ...
                    public void onClick(DialogUtilAction which) {
                        if (which == DialogUtilAction.POSITIVE) {
                            ...
                            if (PosTransHelper.isPosPay(mClearPay.getPayType())) {
                                ...
                                PosTransHelper.gotoPosCancelPayActivity(RetailOrderDetailFragment.this, cancelPay, new PosTransCallBack() {
                                    @Override
