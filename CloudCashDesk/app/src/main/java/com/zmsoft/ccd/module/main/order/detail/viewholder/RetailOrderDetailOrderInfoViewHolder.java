 ...
 */
public class RetailOrderDetailOrderInfoViewHolder extends RetailOrderDetailBaseViewHolder {
    ...
    @BindView(R.id.relative_server_fee)
    RelativeLayout mRelativeServerFee;

    public RetailOrderDetailOrderInfoViewHolder(Context context, View itemView, RecyclerView.Adapter adapter) {
    ...
    protected void bindView(List source, Object bean) {
        ...
        if (bean != null && bean instanceof OrderDetailItem) {
            ...
            fillView(item.getOrderVo(), item.getPayOperator());
        }
    }
    ...
    private void fillView(final OrderItem order, String payOperator) {
        if (order != null) {
            OrderVo orderVo = order.getOrderVo();

            mTextOrderFrom.setText(getContext().getString(R.string.order_from, RetailBillDetailHelper.getRetailOrderFrom(getContext(), orderVo.getOrderFrom())));
            OrderHelper.setOrderDetailPayImage(orderVo.getStatus(), order.getOrderDetailStatus(), mImageOrderState);
            mTextOrderNumber.setText(StringUtils.appendStr(GlobalVars.context.getString(R.string.No), orderVo.getCode()));
            ...
            if (orderVo.getStatus() == OrderHelper.OrderDetail.Order.END_PAY) {
                ...
                mTextOpenedTime.setVisibility(View.VISIBLE);
                mTextOpenTime.setText(String.format(GlobalVars.context.getString(R.string.open_time), TimeUtils.getTimeStr(orderVo.getOpenTime(), TimeUtils.FORMAT_MONTH_DAY_TIME)));
            }

            //流水号
            mTvSerialNumber.setText(getContext().getString(R.string.retail_serial_number, orderVo.getInnerCode()));

            //收银员
            mTvCashier.setText(getContext().getString(R.string.retail_cashier, payOperator));
            ...
            if (needPay > 0) {
                ...
                mLinearNeedPay.setVisibility(View.VISIBLE);
                mTextNeedPayText.setText(getContext().getString(R.string.pay_more));
                mTextNeedPay.setText(FeeHelper.jointMoneyWithCurrencySymbol(UserHelper.getCurrencySymbol()
            ...
            if (OrderHelper.isTakeoutOrder(orderVo.getOrderFrom())) {
                mTextOtherFeeName.setText(getContext().getString(R.string.distribution_fee));
                ...
                mRelativeServerFee.setVisibility(View.VISIBLE);
            } else {
                mRelativeServerFee.setVisibility(View.GONE);
                //mTextOtherFeeName.setText(context.getString(R.string.service_money));
                //mTextServicePrice.setText(String.format(context.getString(R.string.yuan_format), FeeHelper.getDecimalFee(order.getOriginServiceCharge())));
            }
            ...
            if (serviceBill.getOriginLeastAmount() > 0) {
                ...
                mTextLeastPrice.setText(String.format(getContext().getString(R.string.yuan_format), FeeHelper.getDecimalFee(serviceBill.getOriginLeastAmount())));
            } else {
                mRelativeLowestFee.setVisibility(View.GONE);
            }
            mTextTakeOutPrice.setText(FeeHelper.jointMoneyWithCurrencySymbol(UserHelper.getCurrencySymbol()
