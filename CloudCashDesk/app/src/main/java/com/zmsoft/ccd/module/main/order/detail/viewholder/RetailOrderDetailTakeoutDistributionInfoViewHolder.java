 ...
 * </pre>
 */
public class RetailOrderDetailTakeoutDistributionInfoViewHolder extends RetailOrderDetailBaseViewHolder {
    ...
    public RetailOrderDetailTakeoutDistributionInfoViewHolder(Context context, View itemView, RecyclerView.Adapter adapter) {
    ...
    public void fillView(OrderDetailTakeoutInfoVo info) {
        ...
        if (info.getIsThirdShipping() == TakeoutHelper.DELIVERY_INVITE) {
            mTextTimeWay.setText(getContext().getString(R.string.retail_customer_invite_time));
            mTextReceivePeopleNameAndPhone.setText(String.format(getContext().getString(R.string.retail_receive_name_and_phone)
                    , StringUtils.notNull(info.getName())
                    , StringUtils.notNull(info.getPhone())));
            mTextDistributionWay.setText(getContext().getString(R.string.retail_goto_shop_get));

        } else {
            mTextReceivePeopleNameAndPhone.setText(String.format(getContext().getString(R.string.receive_name_and_phone)
                    , StringUtils.notNull(info.getName())
                    , StringUtils.notNull(info.getPhone())));
            mTextDistributionWay.setText(getContext().getString(R.string.send_goods_to_door));
            mTextTimeWay.setText(getContext().getString(R.string.retail_customer_request_time));
        }

        // 配送地址
        if (StringUtils.isEmpty(info.getAddress())
                || info.getIsThirdShipping() == TakeoutHelper.DELIVERY_INVITE) {
