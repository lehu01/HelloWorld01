 ...
 * @time : 2016/12/21 18:30
 */
public class RetailOrderDetailOrderInstanceViewHolder extends RetailOrderDetailBaseViewHolder {

    @BindView(R.id.text_instance_name)
    TextView mTextInstanceName;
    @BindView(R.id.tv_weight_sign)
    TextView mTvWeightSign;
    @BindView(R.id.text_instance_specification)
    TextView mTextInstanceSpecification;
    @BindView(R.id.text_instance_barcode)
    TextView mTextInstanceBarcode;
    @BindView(R.id.text_instance_price)
    ...
    public RetailOrderDetailOrderInstanceViewHolder(Context context, View itemView, RecyclerView.Adapter adapter) {
    ...
    private void fillView(Instance instance) {
        if (instance != null) {
            ...
            if (Base.INT_FALSE == instance.getIsTwoAccount()) { // 非双单位
                mTvWeightSign.setVisibility(View.GONE);
                ...
                if (isReject) {
                    mTextInstanceCount.setText(SpannableStringUtils.getBuilder(getContext()
                            , StringUtils.appendStr(NumberUtils.doubleToStr(instance.getAccountNum())
                                    , StringUtils.isEmpty(instance.getAccountUnit()) ? instance.getUnit() : instance.getAccountUnit()))
                            .setStrikethrough()
                            .create());
                } else {
                    mTextInstanceCount.setText(StringUtils.appendStr(NumberUtils.doubleToStr(instance.getAccountNum())
                            , StringUtils.isEmpty(instance.getAccountUnit()) ? instance.getUnit() : instance.getAccountUnit()));
                }
            } else {  // 双单位菜肴
                mTvWeightSign.setVisibility(View.VISIBLE);
                ...
                // 设置中划线
                if (isReject) {
//                    mTextInstanceCount.setText(SpannableStringUtils.getBuilder(context
            ...
            if (Base.SHORT_TRUE == instance.getIsWait()) {
                ...
                if (isReject) {
                    ...
                    if (!StringUtils.isEmpty(instance.getMenuCode())) {
                        mTextInstanceBarcode.setText(SpannableStringUtils.getBuilder(getContext(), instance.getName())
                                .setStrikethrough()
                                .create());
                    }
                } else {
                    ...
                    if (!StringUtils.isEmpty(instance.getMenuCode()))
                        mTextInstanceBarcode.setText(instance.getMenuCode());
                    else
                        mTextInstanceBarcode.setText(getContext().getString(R.string.retail_no_barcode_label));
                }
            }

            // 设置中划线
            if (isReject) {
                mTextInstancePrice.setText(SpannableStringUtils.getBuilder(getContext()
                        , FeeHelper.jointMoneyWithCurrencySymbol(UserHelper.getCurrencySymbol()
                                , FeeHelper.getDecimalFee(instance.getFee())))
                        .setStrikethrough()
                        .create());
            } else {
                mTextInstancePrice.setText(FeeHelper.jointMoneyWithCurrencySymbol(UserHelper.getCurrencySymbol()
                        , FeeHelper.getDecimalFee(instance.getFee())));
            }
...
//                // 设置中划线
//                if (isReject) {
//                    mTextInstanceSpecification.setText(SpannableStringUtils.getBuilder(context, specDetailStr)
//                            .setStrikethrough()
//                            .create());
//                } else {
