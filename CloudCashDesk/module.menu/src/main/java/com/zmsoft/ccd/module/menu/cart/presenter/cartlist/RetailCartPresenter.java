...
public class RetailCartPresenter implements RetailCartContract.Presenter {
    private RetailCartContract.View mView;
    private CartRepository mRepository;
    private IMenuSource mMenuRepository;

    @Inject
    public RetailCartPresenter(RetailCartContract.View view, CartRepository repository) {
        mView = view;
...
//        mView.showLoading(GlobalVars.context.getString(R.string.dialog_waiting), false);
    ...
    @Override
    public void submitOrder(SubmitOrderRequest request, List<RetailCartRecyclerItem> cartRecyclerItemList) {
    ...
    public void saveCustomMenuToCart(String seatCode, ItemVo itemVo, int peopleCount) {
        ...
        if (null != itemVo) {
            memo = itemVo.getMemo();
            num = itemVo.getNum();
            itemVo.setAccountNum(num); //设置结账数量
        }
    ...
    private boolean isCartValid(List<RetailCartRecyclerItem> cartRecyclerItemList) {
        if (null != cartRecyclerItemList && !cartRecyclerItemList.isEmpty()) {
            ...
            for (int i = 0; i < cartRecyclerItemList.size(); i++) {
                RetailCartRecyclerItem cartRecyclerItem = cartRecyclerItemList.get(i);
                if (null != cartRecyclerItem) {
                    if (cartRecyclerItem.getItemType() == CartRecyclerItem.ItemType.TYPE_COMBO_FOOD) {
                        RetailCartComboFoodRecyclerItem comboItem = cartRecyclerItem.getRetailCartComboFoodRecyclerItem();
                        if (null != comboItem) {
    ...
    private String getCustomMenuJson(ItemVo itemVo) {
        ...
        //结账单位与点菜单位相同
        boolean isValid = (!TextUtils.isEmpty(itemVo.getUnit()) && !TextUtils.isEmpty(itemVo.getAccountUnit())
                && itemVo.getUnit().equals(itemVo.getAccountUnit()));
        if (isValid) {
            //结账数量
            item.setAccountNum(0d);
