...
import com.zmsoft.ccd.module.menu.cart.adapter.cartdetail.RetailCartDetailAdapter;
...
import com.zmsoft.ccd.module.menu.helper.RetailCardDetailDataMapLayer;
...
public class RetailCartDetailFragment extends BaseListFragment implements RetailCartDetailContract.View {
    ...
    EditFoodNumberView mEditFoodWeightView;
    ...
    private RetailCartDetailContract.Presenter mPresenter;
    /**
     * 菜肴ID
     */
    ...
    public static RetailCartDetailFragment newInstance(String menuId, int detailType, ItemVo itemVo
        ...
        RetailCartDetailFragment fragment = new RetailCartDetailFragment();
    ...
    protected int getLayoutId() {
        return R.layout.module_menu_fragment_retail_cart_detail_layout;
    }
    ...
    protected void initView(View view, Bundle savedInstanceState) {
        ...
        if (null != mItemVo) {
            if (mItemVo.getNum() > 0) {
                ...
                foodWeight = mItemVo.getAccountNum();
            }
        }
...
//                                    }
                            ...
                            mNormalMenuVo.getMenu().setAccountNum(foodWeight);
                            mPresenter.modifyCart(mSeatCode, mOrderId, mMenuId, mCartRecyclerItems
                                    , mItemVo, mNormalMenuVo, mKindTasteList, foodNum, mDetailType);
                        }
                    }
    ...
    private void initFoodNumLIstener() {
            ...
            public void onClickEdit(double numberValue) {
                if (mNormalMenuVo != null && mNormalMenuVo.getMenu() != null) {
                    ...
                    showEditValueDialog(mNormalMenuVo,
                            mNormalMenuVo.getMenu().getStartNum(),
                            numberValue,
    ...
    private void initWeightListener() {
        mEditFoodWeightView.setOnInputDone(new EditFoodNumberView.OnInputDone() {
            @Override
            public void onDone(double numberValue) {
                mEditFoodWeightView.setNumberText(numberValue);
                foodWeight = numberValue;
                handleModifyEvent();
            }
        });

        mEditFoodWeightView.setSoftBackListener(new EditFoodNumberView.SoftBackListener() {
            @Override
            public void onSoftBack(TextView textView, double inputNum) {
                //检查起点分数
                if (mDetailType != CartHelper.FoodDetailType.COMBO_CHILD_DETAIL
                        && mDetailType != CartHelper.FoodDetailType.COMBO_MUST_SELECT_CHILD_DETAIL
                        && !CartHelper.checkStartNum(getStartNum(), inputNum)) {
                    if (mItemVo == null) {
                        mEditFoodWeightView.setNumberText(getStartNum());
                        ToastUtils.showShortToast(getContext(),
                                getString(R.string.module_menu_check_start_num, getMenuName(), getStartNum(), getUnit()));
                    } else {
                        if (inputNum != 0) {
                            mEditFoodWeightView.setNumberText
        ...
        mEditFoodWeightView.setOnEditTextChangeListener(new EditFoodNumberView.OnEditTextChangeListener() {
            @Override
        ...
        mEditFoodWeightView.getEditText().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mEditFoodWeightView.getEditText().clearFocus();
                mEditFoodWeightView.getEditText().requestFocus();
            }
        });
    ...
    protected BaseListAdapter createAdapter() {
        return new RetailCartDetailAdapter(getActivity(), null);
    }
    ...
    @Override
    public void setPresenter(RetailCartDetailContract.Presenter presenter) {
        ...
        if (baseMenuVo != null) {
            Subscription mDataMapSubscription = RxUtils.fromCallable(new Callable<List<CartDetailRecyclerItem>>() {
                ...
                public List<CartDetailRecyclerItem> call() {
                    mNormalMenuVo = RetailCardDetailDataMapLayer.getNormalMenuVo(baseMenuVo);
                    mKindTasteList = baseMenuVo.getKindTasteList();
                    initFoodNumView();
                    if (mDetailType == CartHelper.FoodDetailType.COMBO_MUST_SELECT_CHILD_DETAIL) {
                        return RetailCardDetailDataMapLayer.getComboDetailList(getActivity(), baseMenuVo
                                , mParamSuitSubMenu, mDetailType, mSpecId);
                    } else {
                        return RetailCardDetailDataMapLayer.getCartDetailList(getActivity(), baseMenuVo
    ...
    private void initBottomView() {
        if (isWeight()) {
            ...
            mEditFoodWeightView.setNumberText(foodWeight);
            mButtonSure.setVisibility(View.INVISIBLE);
            mEditFoodWeightView.setVisibility(View.VISIBLE);
            mEditFoodWeightView.getEditText().setSelectAllOnFocus(true);
            mEditFoodWeightView.getEditText().requestFocus();
            mRlFoodWeight.setVisibility(View.VISIBLE);
            mTvWeightUnit.setText(mNormalMenuVo.getMenu().getAccount());
            mTextSure.setVisibility(View.VISIBLE);
            mTextSure.setText((String.format(context.getResources().getString(R.string.module_menu_cartdetail_ordered_num)
                    , foodWeight, mNormalMenuVo.getMenu().getAccount())));
        } else {
            mEditFoodNumberView.setVisibility(View.VISIBLE);
    ...
    private boolean isWeight() {
        if (null == mNormalMenuVo) return false;
...
//                && !menu.getBuyAccount().equals(menu.getAccount());
        return ((menu.getIsTwoAccount() == CartHelper.TwoAccountKind.KIND_2Account)); //是否有双单位
    }
