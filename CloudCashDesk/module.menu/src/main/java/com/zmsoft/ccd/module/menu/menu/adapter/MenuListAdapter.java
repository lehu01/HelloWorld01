...
public class MenuListAdapter extends BaseListAdapter {
    ...
    public static MenuListAdapter createImageAdapter(Context context, AdapterClick adapterClick) {
        return new MenuListAdapter(context, true, adapterClick);
    }

    private MenuListAdapter(Context context, boolean isShowImage, AdapterClick adapterClick) {
        super(context, null);
    ...
    public BaseHolder onMyCreateViewHolder(ViewGroup parent, int viewType) {
        switch (viewType) {
            ...
            case LAYOUT_TYPE_GROUP:
                return new GroupCategoryHolder(mContext, inflateLayout(R.layout.module_menu_item_group_category, parent));
            case LAYOUT_TYPE_MENU:
                return new MenuHolder(mContext, inflateLayout(isShowImage ?
                        R.layout.module_menu_item_menu_with_image : R.layout.module_menu_item_menu_without_image, parent)
                        , mSeatCodeStatus, mAdapterClick, this);
            case LAYOUT_TYPE_SPECIFICATION:
                return new SpecificationHolder(getContext(),
                        inflateLayout(R.layout.module_menu_item_menu_specification, parent), mAdapterClick);
        }
    ...
    static class MenuHolder extends BaseHolder {
        ...
        @BindView(R2.id.text_require_tip)
        ...
        MenuHolder(Context context, View itemView, int seatCodeStatus, AdapterClick adapterClick, MenuListAdapter menuListAdapter) {
            super(context, itemView);
            ButterKnife.bind(this, itemView);
        ...
        protected void bindView(List source, Object bean) {
            if (bean instanceof MenuVO) {
                ...
                if (menuVO.getSpecificationVOs() != null
                        && menuVO.getSpecificationVOs().size() == 1) {
                ...
                //是否是套餐
                mTextSuitFoodFlag.setVisibility(menu.isSuit() ? View.VISIBLE : View.GONE);
                //是否显示必选商品标识
