...
public class RetailCartBaseViewholder extends BaseHolder {
    ...
    protected RetailCartRecyclerItem mCartRecyclerItem;
    protected View mItemView;
    protected DialogUtil mDialogUtil;

    public RetailCartBaseViewholder(Context context, View itemView) {
    ...
    protected void bindView(List source, Object bean) {
        if (null != bean && bean instanceof RetailCartRecyclerItem) {
            mCartRecyclerItem = (RetailCartRecyclerItem) bean;
        ...
        if (source.size() == 1) {
            ...
            switch (itemType) {
                ...
                case RetailCartRecyclerItem.ItemType.TYPE_COMBO_FOOD:
                    ...
                    if (null != object && object instanceof RetailCartRecyclerItem) {
                        if (((RetailCartRecyclerItem) object).getItemType() != RetailCartRecyclerItem.ItemType.TYPE_NORMAL_FOOD
                                && ((RetailCartRecyclerItem) object).getItemType() != RetailCartRecyclerItem.ItemType.TYPE_COMBO_FOOD) {
                            setSemicircle(false, true);
                        } else {
                    ...
                    //零售不展示类目，第一个商品展示semicircle
                    if (position >= 1) {
                        object = source.get(position - 1);
                        if (null != object && object instanceof RetailCartRecyclerItem) {
                            if (((RetailCartRecyclerItem) object).getItemType() == RetailCartRecyclerItem.ItemType.TYPE_ORDER_INFO) {
                                setSemicircle(true, false);
                            }
                        }
                    }
