...
import com.zmsoft.ccd.module.menu.cart.adapter.cartlist.RetailCartAdapter;
...
import com.zmsoft.ccd.module.menu.helper.RetailDataMapLayer;
...
public class RetailCartFragment extends BaseListFragment implements RetailCartContract.View {
    ...
    private RetailCartContract.Presenter mPresenter;
    /**
     * 菜单页传递过来的参数
     */
    ...
    private List<RetailCartRecyclerItem> mCartRecyclerItemList;

    public static RetailCartFragment newInstance(OrderParam createOrderParam, int from, DinningTableVo dinningTableVo) {
        ...
        RetailCartFragment fragment = new RetailCartFragment();
    ...
    protected int getLayoutId() {
        return R.layout.module_menu_fragment_retail_cart_layout;
    }
    ...
    protected void initListener() {
        ...
        RxView.clicks(mTextGoOn).throttleFirst(1, TimeUnit.SECONDS)
                .subscribe(new Action1<Void>() {
                    @Override
                    public void call(Void aVoid) {
                        Intent intent = new Intent(getActivity(), RetailMenuListActivity.class);
                        intent.putExtra(RouterPathConstant.RetailMenuList.EXTRA_CREATE_ORDER_PARAM, mCreateOrderParam);
                        intent.putExtra(RouterPathConstant.RetailCart.EXTRA_FROM, RouterPathConstant.RetailCart.EXTRA_FROM_HANG_UP_ORDER);
                        startActivityForResult(intent, RouterPathConstant.RetailCart.REQUEST_CODE_TO_MENU_LIST);
                    }
                });
    ...
    protected BaseListAdapter createAdapter() {
        return new RetailCartAdapter(getActivity(), null);
    }
    ...
    @Override
    public void setPresenter(RetailCartContract.Presenter presenter) {
    ...
    public void showCart(final DinningTableVo dinningTableVo) {
        ...
        if (dinningTableVo != null) {
            Subscription subscription = RxUtils.fromCallable(new Callable<List<RetailCartRecyclerItem>>() {
                @Override
                public List<RetailCartRecyclerItem> call() {
                    mCartRecyclerItemList = RetailDataMapLayer.getCartList(getActivity(), dinningTableVo
                            , mCreateOrderParam);
                    ...
                    .onTerminateDetach()
                    .subscribe(new Action1<List<RetailCartRecyclerItem>>() {
                        @Override
                        public void call(List<RetailCartRecyclerItem> cartRecyclerItems) {
                            cleanAll();
    ...
    public void queryCartError(ErrorBody body) {
        ...
        if (null != body && !TextUtils.isEmpty(body.getErrorCode()) && body.getErrorCode().equals(ErrorBizHttpCode.ERR_CART106)) {
            showContentView();
            enableRefresh();
            RxUtils.fromCallable(new Callable<List<RetailCartRecyclerItem>>() {
                @Override
                public List<RetailCartRecyclerItem> call() {
                    mCartRecyclerItemList = RetailDataMapLayer.getExpiredCartList();
                    return mCartRecyclerItemList;
                }
            }).observeOn(AndroidSchedulers.mainThread())
                    .subscribeOn(AndroidSchedulers.mainThread())
            ...
            if (isToAddFood) {
                ...
                if (submitOrderVo.isOpenTable()) {
                    ...
                    if (null != mCreateOrderParam && !TextUtils.isEmpty(mCreateOrderParam.getOrderId())) {
                        ...
                        //如果是加菜消息，打印加菜单
                        if (null != submitOrderVo.getInstanceIds() && !submitOrderVo.getInstanceIds().isEmpty()) {
//                            CcdPrintHelper.printAddInstance(getActivity()
//                                    , PrintData.TYPE_SELF_ORDER
//                                    , PrintData.BIZ_TYPE_PRINT_ADD_INSTANCE
//                                    , UserHelper.getEntityId(), UserHelper.getUserId()
//                                    , submitOrderVo.getInstanceIds()
//                                    , mCreateOrderParam.getSeatCode()
//                                    , submitOrderVo.getOrderId());
//                            // 打印标签
...
//                                , submitOrderVo.getOrderId(), UserHelper.getUserId(), UserHelper.getEntityId()
//                                , mCreateOrderParam.getSeatCode());
//                        // 标签打印：点菜单
...
//                                , submitOrderVo.getOrderId(), UserHelper.getUserId(), UserHelper.getEntityId());
                    ...
                    if (isToAddFood) {
                        ...
                        MRouter.getInstance().build(RouterPathConstant.RetailOrderDetail.PATH)
                                .putString(RouterPathConstant.RetailOrderDetail.EXTRA_ORDER_ID
                                        , submitOrderVo.getOrderId())
                                .navigation(getActivity());
                        clearOtherActivity();
                    } else {
                        if (submitOrderVo.isTurnToFindOrder()) {
                            MRouter.getInstance().build(RouterPathConstant.PATH_RETAIL_MAIN_ACTIVITY)
...
//                                    .navigation(getActivity());
                            //跳转到收款页面
                            gotoReceiptActivity(submitOrderVo.getOrderId());
                            clearOtherActivity();
                        }
                    }
                }
     ...
     * 跳转收款页面
     */
    private void gotoReceiptActivity(String orderId) {
        MRouter.getInstance().build(RouterPathConstant.Receipt.PATH)
                .putString(RouterPathConstant.Receipt.EXTRA_ORDER_ID, orderId)
                .navigation(getActivity());
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                case RouterPathConstant.RetailCart.REQUEST_CODE_TO_MENU_LIST: //继续点菜时，从菜单列表返回到购物车，需要刷新购物车
                    refreshCart();
                    break;
                default:
                    break;
    ...
    private void submitOrder(String orderId) {
        if (BatchPermissionHelper.getPermission(Permission.OrderBegin.ACTION_CODE)) {
            ...
            SubmitOrderRequest request = SubmitOrderRequest.createRequestForRetail(
            ...
            showToast(getString(R.string.module_menu_retail_permission_order_begin_));
        }
    ...
    private void goToModifyOrder() {
        MRouter.getInstance().build(RouterPathConstant.RetailRemark.PATH_REMARK)
                .putString(RouterPathConstant.RetailRemark.EXTRA_REMARK, mDinningTableVo.getMemo())
                .putString(RouterPathConstant.RetailRemark.EXTRA_SEAT, mCreateOrderParam.getSeatCode())
                .navigation(getActivity(), RetailCartActivity.RESULT_REMARK_CODE);
    }
    ...
    private void clearOtherActivity() {
        ...
        RouterActivityManager.get().finishActivity(RouterPathConstant.RetailMenuList.PATH);
        if (mFrom == RouterPathConstant.Cart.EXTRA_FROM_HANG_UP_ORDER) {
            RouterActivityManager.get().finishActivity(RouterPathConstant.RetailHangUpOrderList.PATH);
        }
        if (isHostActive()) {
            getActivity().finish();
        }
