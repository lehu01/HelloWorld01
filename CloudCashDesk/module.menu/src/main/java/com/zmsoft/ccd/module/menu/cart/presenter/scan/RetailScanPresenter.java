...
public class RetailScanPresenter implements RetailScanContract.Presenter {

    private RetailScanContract.View mView;
    ...
    @Inject
    public RetailScanPresenter(RetailScanContract.View view, MenuRepository repository) {
        mView = view;
    ...
    @Override
    public void queryMenuListByCode(MenuRetailQuery menuRetailQuery) {
        mView.showLoading(GlobalVars.context.getString(R.string.dialog_waiting), false);
        mRepository.queryMenuListByEntityId(menuRetailQuery).subscribe(new Action1<List<BoMenu>>() {
            @Override
            public void call(List<BoMenu> boMenuList) {
                if (null == mView) {
                    return;
                }
                mView.hideLoading();
                mView.showMenuList(boMenuList);
            }
    ...
    private List<CartItem> getCartItems(String menuId, NormalMenuVo normalMenuVo) {
        ...
        if (null != normalMenuVo.getMenu()) {
            ...
            mCartItem.setAccountNum(1.0); //零售商品使用一个结账单位
