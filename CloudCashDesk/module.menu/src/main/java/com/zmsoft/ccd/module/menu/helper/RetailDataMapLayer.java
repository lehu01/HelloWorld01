...
public class RetailDataMapLayer extends BaseDataHelper {
    /**
     * 购物车列表界面
     *
     ...
     * @return 界面展示需要的数据模型
     */
    public static List<RetailCartRecyclerItem> getCartList(Context context, DinningTableVo dinningTableVo
            , OrderParam createOrderParam) {
        if (null == dinningTableVo)
            return null;
        List<RetailCartRecyclerItem> cartRecyclerItemList = new ArrayList<>();
        //order
        RetailCartRecyclerItem orderRecyclerItem = new RetailCartRecyclerItem();
        orderRecyclerItem.setItemType(RetailCartRecyclerItem.ItemType.TYPE_ORDER_INFO);

        RetailCartOrderRecyclerItem cartOrderRecyclerItem = new RetailCartOrderRecyclerItem();
        ...
        orderRecyclerItem.setRetailCartOrderRecyclerItem(cartOrderRecyclerItem);
     ...
     * @return 界面展示需要的数据模型
     */
    public static List<RetailCartRecyclerItem> getExpiredCartList() {
        List<RetailCartRecyclerItem> cartRecyclerItemList = new ArrayList<>();
        //order
        RetailCartRecyclerItem orderRecyclerItem = new RetailCartRecyclerItem();
     ...
     */
    private static void handleCartFoodList(Context context, List<RetailCartRecyclerItem> cartRecyclerItemList
            , DinningTableVo dinningTableVo, RetailCartOrderRecyclerItem cartOrderRecyclerItem
            , OrderParam createOrderParam) {
        //合计总金额
        double amountFee = 0;
        //菜详情（必选商品、热菜、冷菜等）
        ...
        if (null != kindUserCarts && !kindUserCarts.isEmpty()) {
            for (int i = 0; i < kindUserCarts.size(); i++) {
                ...
                if (null != kindUserCartVo) {
                    //category
                    //零售不展示分类，暂时隐藏
//                    RetailCartRecyclerItem categoryRecyclerItem = new RetailCartRecyclerItem();
//                    categoryRecyclerItem.setItemType(RetailCartRecyclerItem.ItemType.TYPE_CATEGORY);

//                    RetailCartCategoryRecyclerItem cartCategoryRecyclerItem = new RetailCartCategoryRecyclerItem();
//                    if (null != kindUserCartVo.getKindVo()) {
//                        cartCategoryRecyclerItem.setCategoryName(kindUserCartVo.getKindVo().getName());
//                    }
//
//                    categoryRecyclerItem.setRetailCartCategoryRecyclerItem(cartCategoryRecyclerItem);
//                    cartRecyclerItemList.add(categoryRecyclerItem);

                    //food
                    amountFee += handleNormalAndComboFood(context, kindUserCartVo, null
    ...
    private static double handleNormalAndComboFood(Context context, KindUserCartVo kindUserCartVo
            , RetailCartCategoryRecyclerItem cartCategoryRecyclerItem, List<RetailCartRecyclerItem> cartRecyclerItemList
            , OrderParam createOrderParam) {
        //合计总金额
        double amountFee = 0;
        //food
                ...
                if (null != itemVo) {
                    ...
                    switch (itemVo.getKind()) {
                        ...
                        case CartHelper.CartFoodKind.KIND_CUSTOM_FOOD:
                            RetailCartRecyclerItem normalFoodRecyclerItem = new RetailCartRecyclerItem();
                            normalFoodRecyclerItem.setItemType(RetailCartRecyclerItem.ItemType.TYPE_NORMAL_FOOD);

                            RetailCartNormalFoodRecyclerItem cartNormalFoodRecyclerItem = new RetailCartNormalFoodRecyclerItem();
                            ...
                            cartNormalFoodRecyclerItem.setDoubleUnits(itemVo.getDoubleUnits());
                            ...
                            normalFoodRecyclerItem.setRetailCartNormalFoodRecyclerItem(cartNormalFoodRecyclerItem);
                            cartRecyclerItemList.add(normalFoodRecyclerItem);
                            break;
                        case CartHelper.CartFoodKind.KIND_COMBO_FOOD:
                            RetailCartRecyclerItem comboFoodRecyclerItem = new RetailCartRecyclerItem();
                            comboFoodRecyclerItem.setItemType(CartRecyclerItem.ItemType.TYPE_COMBO_FOOD);

                            RetailCartComboFoodRecyclerItem cartComboFoodRecyclerItem = new RetailCartComboFoodRecyclerItem();
                            //将该菜的原始数据信息保存在recyclerItem内，供跳转到购物车详情等界面时候使用
                            ...
                            comboFoodRecyclerItem.setRetailCartComboFoodRecyclerItem(cartComboFoodRecyclerItem);
    ...
    private static String getSummaryInfo(Context context, DinningTableVo dinningTableVo) {
        ...
        int count = 0;
        for (KindUserCartVo kindUserCartVo : dinningTableVo.getKindUserCarts()) {
            for (ItemVo itemVo : kindUserCartVo.getItemVos()) {
                if (itemVo.getDoubleUnits() == 1) {
                    count += 1;
                } else {
                    count += itemVo.getAccountNum();
                }
            }
        }
        summaryInfoBuilder.append(String.format(context.getResources().getString(R.string.module_menu_retail_summary_food_num), count));
     ...
     * @return
     */
    private static String getComboFoodMakeName(Context context, ItemVo itemVo, RetailCartComboFoodRecyclerItem cartComboFoodRecyclerItem) {
        StringBuilder makeNameBuilder = new StringBuilder();
        //子菜
        List<Item> feedList = itemVo.getChildItems();
        if (null != feedList && !feedList.isEmpty()) {
