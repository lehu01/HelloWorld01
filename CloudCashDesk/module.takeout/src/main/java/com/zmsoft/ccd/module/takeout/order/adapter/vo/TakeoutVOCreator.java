...
public class TakeoutVOCreator {
    ...
    public static OrderMainHolderVO createMainHolderVO(Context context, Takeout takeout) {
        ...
        mainHolderVo.setOrderStatus(context.getString(TakeoutUtils.getStatusShowName(takeout)));
        ...
        mainHolderVo.setNextMenuValue(context.getString(TakeoutUtils.getOrderNextMenu(takeout)));

        mainHolderVo.setOrderOriginalImage(TakeoutUtils.getOrderFromImage(takeout));

        mainHolderVo.setAppointmentFlagText(context.getResources().getString(R.string.module_takeout_reserve_flag));

        String prefix;
        if (takeout.getSendType() == TakeoutConstants.SendType.SELF_TAKE) {
            mainHolderVo.setOrderDeliveryWay(context.getResources().getString(R.string.module_takeout_take_self));
            mainHolderVo.setOrderTakeTime(context.getResources().getString(R.string.module_takeout_take_time_self, takeout.getSendTime()));
            prefix = (context.getResources().getString(R.string.module_takeout_prefix_receiver_self));
        } else {
    ...
    public static OrderDescHolderVO getOrderDescHolderVO(Context context, Takeout takeout) {
        ...
        if (peopleCount != 0) {
            descHolderVO.setPersonNum(context.getString(R.string.module_takeout_order_people_num, peopleCount));
        }
    ...
    public static List<OrderFoodHolderVO> getOrderFoodHolderVO(Context context, Takeout takeout) {
        if (takeout.getTakeoutOrderDetailVo() != null &&
            ...
            for (Takeout.TakeoutInstance instance : foods) {
                ...
                foodHolderVO.setTwoAccount(MenuUtils.isTwoAccount(instance.getUnit(), instance.getAccountUnit()));
    ...
    public static OrderPayInfoHolderVO getOrderPayInfoHolderVO(Context context, Takeout takeout) {
        if (takeout.getTakeoutOrderDetailVo() != null) {
            ...
            if (takeout.getTakeoutOrderDetailVo().getOutFee() != 0) {
                ...
                CharSequence freightStr = context.getString(R.string.module_takeout_bar_pay_freight
                        , FeeHelper.jointMoneyWithCurrencySymbol(UserHelper.getCurrencySymbol(), freight));
    ...
    public static OrderInfoHolderVO getOrderInfoHolderVO(Context context, Takeout takeout) {
        if (takeout.getTakeoutOrderDetailVo() != null) {
            ...
            orderInfoHolderVO.setOrderInfo(context.getString(R.string.module_takeout_order_order_info,
                    ...
                    takeout.getTakeoutOrderDetailVo().getGlobalCode()));
